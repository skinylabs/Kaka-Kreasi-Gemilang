...
class RuleController extends Controller
{
    ...
    public function import(Request $request, TataTertib $tataTertib)
    {
        ...
        return redirect()->route('tatatertib.index', $tataTertib->id)->with('success', 'Tata Tertib berhasil ditambahkan.');
    }
    ...
    {
        $tataTertib = TataTertib::where('tata_tertib_id', $tataTertib->id)->get();
        return view('pages.backend.pages.tatatertib.partials.edit', compact('tataTertib', 'rule'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(TataTertib $tataTertib, Request $request)
    {
        // Validasi input
        $request->validate([
    ...
    {
        $rule->delete();
        return redirect()->route('tour.index', $tataTertib->id)->with('success', 'Tata Tertib berhasil dihapus.');
    }
}
