...
@section('content')
    ...
    <a href="{{ route('tatatertib.create', $tour->id) }}" class="btn btn-primary">Tambah Transportasi</a>

    <form action="{{ route('tatatertib.import', $tour->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <tbody>
            @foreach ($tataTertib as $t)
                <tr>
                    ...
                    <td class="border border-gray-300 p-2">{{ $t->id }}</td>
                    <td class="border border-gray-300 p-2">{{ $t->title }}</td>


                    <td class="border border-gray-300 p-2">
                        <div class="flex justify-evenly">
                            <a href="{{ route('tatatertib.show', $t->id) }}">View</a>
                            <a href="{{ route('tatatertib.edit', $t->id) }}">Edit</a>
                            <button class="text-red-500 delete-btn" data-tatatertib-id="{{ $t->id }}">Delete</button>
                        </div>
                    </td>
                </tr>
            @endforeach
